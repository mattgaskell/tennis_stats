with wins as (
	...
	min(winner_age_yrs) as winner_age_yrs_start_yr,
	count(*) as wins,
	...
	sum(winner_receiving_pts) as winner_receiving_pts,
	sum(winner_receiving_pts_won) as winner_receiving_pts_won,
	sum(winner_serve_games) as winner_serve_games,
	sum(winner_serve_games_won) as winner_serve_games_won,
	sum(winner_receiving_games) as winner_receiving_games,
	sum(winner_receiving_games_won) as winner_receiving_games_won,
...
losses as (
	...
	min(loser_age_yrs) as loser_age_yrs_start_yr,
	count(*) as losses,
	...
	sum(loser_receiving_pts) as loser_receiving_pts,
	sum(loser_receiving_pts_won) as loser_receiving_pts_won,
	sum(loser_serve_games) as loser_serve_games,
	sum(loser_serve_games_won) as loser_serve_games_won,
	sum(loser_receiving_games) as loser_receiving_games,
	sum(loser_receiving_games_won) as loser_receiving_games_won,
...
w.player_name,
w.player_height_cm,
case when winner_age_yrs_start_yr < loser_age_yrs_start_yr 
	then winner_age_yrs_start_yr else loser_age_yrs_start_yr end as player_age_yrs_start_yr,
...
100.*(winner_receiving_pts_won + loser_receiving_pts_won)/(winner_receiving_pts + loser_receiving_pts) as pc_receiving_pts_won,
100.*(winner_pts_won + loser_pts_won)/(w.pts + l.pts) as pc_pts_won,
100.*(winner_serve_games_won + loser_serve_games_won)/(winner_serve_games + loser_serve_games) as pc_serve_games_won,
100.*(winner_receiving_games_won + loser_receiving_games_won)/(winner_receiving_games + loser_receiving_games) as pc_receiving_games_won,
100.*(winner_games_won + loser_games_won)/(w.games + l.games) as pc_games_won,
100.*(winner_1st_serves_in + loser_1st_serves_in)/(winner_serve_pts + loser_serve_pts) as pc_1st_serve_in,
100.*(winner_dfs + loser_dfs)/(winner_serve_pts + loser_serve_pts) as pc_dfs
